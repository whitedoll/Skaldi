"""자식 프로세스(translate.py)의 진행 상황을 부모(GUI)에게 알리는 한 줄 프로토콜.

GUI 는 파이프라인을 자식 프로세스로 돌리고 stdout 만 읽는다. 사람이 읽는 로그만으로는
"몇 장 중 몇 장째"를 알 수 없으므로, 환경변수 ``MANGATRANS_PROGRESS=1`` 이 켜져 있을 때만
...
def enabled() -> bool:
    ...
    return os.environ.get("MANGATRANS_PROGRESS") == "1"
